/**
 * AdminTimelineScreen — full chronological activity feed across the WHOLE
 * system (all investors + all Miller Group deals). Navigated to from the
 * "ציר זמן מלא" link on the admin dashboard. Includes a back button that
 * works on mobile and desktop.
 *
 * Uses /api/timeline/admin-feed with no investor scoping and role=admin so
 * renovation-payment events are included.
 */
...
export function AdminTimelineScreen() {
  ...
  const filtered = useMemo(() => {
    ...
    if (filter === 'renovation') return events.filter(e => e.kind === 'renovation-payment');
    return events;
  }, [events, filter]);
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'var(--bg-base)', overflow: 'hidden' }}>
      <div className="desktop-page-title">
        <div className="subtitle">כל האירועים במערכת</div>
        <h1>ציר זמן מלא</h1>
      </div>

      <div className="screen-header" style={{ padding: '16px 20px 10px', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
        <button
          onClick={goBack}
          style={{
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '8px 20px 20px' }}>
        {/* Filter chips — wrap so every option is visible */}
        <div style={{
          display: 'flex', gap: 6, padding: '2px 0 12px',
          ...
          {([
            ...
            { key: 'renovation' as FilterKey, label: '🔨 תשלומים' },
          ]).map(c => (
            <button
              key={c.key}
